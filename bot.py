...
# Функція для автоматичної реєстрації користувача
@dp.message()
async def auto_register_user(message: Message):
    user_id = message.from_user.id
    username = message.from_user.username

    # Якщо користувач не зареєстрований, додаємо його
...
async def show_balance(message: Message):
    args = message.text.split()
    if len(args) == 1:  # Перегляд балансу власного
        user_id = message.from_user.id
        ...
        username = args[1].lstrip('@')
        user = await get_user_info_by_username(username)
        if user:
            await message.answer(f"💰 Баланс @{username}: {user['points']} балів.")
        else:
            await message.answer(f"👤 Користувача @{username} не знайдено.")
    else:
        await message.answer("⚠️ Невірний формат команди. Використовуйте: /balance @username")
